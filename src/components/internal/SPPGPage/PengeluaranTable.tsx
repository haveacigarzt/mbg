...
const columns = [
  ...
  columnHelper.accessor((row) => row.jumlah * row.harga_satuan, {
    id: "total",
    header: "Total",
    enableSorting: true,
    cell: (info) => formatRupiah(info.getValue()),
...
const PengeluaranTable = ({ sppg_id, pengeluaran }: Props) => {
    ...
    <div className="flex flex-col gap-4">
          ...
          <thead>
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id} className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="px-4 py-3 text-left text-xs text-gray-400 tracking-widest font-semibold">
                  AKSI
                </th>
              </tr>
            ))}
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-gray-400">
          Halaman {page} dari {metadata.last_page} —{" "}
          <span className="font-bold text-gray-600">
            TOTAL {metadata.total_records} DATA
          </span>
        </p>
        <div className="flex items-center gap-1">
          <button
