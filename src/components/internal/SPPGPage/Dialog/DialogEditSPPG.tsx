...
export function DialogEditSPPG({ data, kecamatan, kelurahan, onSPPGUpdate }: Props) {
      ...
      >
        <form onSubmit={handleSubmit}>
          <DialogHeader className="gap-0">
            <DialogTitle className="text-lg font-semibold">Edit data SPPG</DialogTitle>
            <DialogDescription>Ubah data SPPG anda. Klik simpan saat selesai.</DialogDescription>
          </DialogHeader>
            ...
            <div className="flex flex-1 flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <NativeSelect id="kelurahan" onChange={(e) => updateField('kelurahan_id', Number(e.target.value))} defaultValue={form.kelurahan_id || 0} className="w-full">
                    <NativeSelectOption disabled value="" className="text-center">
                      --- Pilih Kelurahan ---
                    </NativeSelectOption>
                    {kelurahanNew.map((el) => (
            ...
            <div className="flex flex-1 flex-col gap-5">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input id="kepala_sppg" placeholder="Kepala SPPG" value={form.kepala_sppg} onChange={(e) => updateField('kepala_sppg', e.target.value)} required />
                </div>
                <div>
                  <Label htmlFor="kapasitas" className="mb-1">
                    Kapasitas Porsi
                  </Label>
                  <Input id="kapasitas" type="number" placeholder="Kapasitas Porsi" value={form.kapasitas_porsi} onChange={(e) => updateField('kapasitas_porsi', Number(e.target.value))} required />
                </div>
              </div>
              <div className="flex items-center justify-between border p-3 rounded">
                <span>Status Aktif</span>
                <Switch checked={form.status_aktif} onCheckedChange={(val) => updateField('status_aktif', val)} className="data-[state=checked]:bg-blue-600" required />
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 mt-2">
                  {form.sosmed_url.map((url, index) => (
                    <div key={index} className="flex gap-2">
                      ...
                      <Button type="button" variant="destructive" onClick={() => removeSosmed(index)}>
                        Hapus
                      </Button>
                    </div>
                  ))}
                ...
                <Button type="button" variant="outline" className="mt-2" onClick={addSosmed}>
                  Tambah URL
                </Button>
              </div>
            </div>
