...
const DialogTambahPengiriman = ({
  ...
  function handleCheckboxChange(
    ...
    setForm((prev) => {
      ...
      return {
        ...prev,
        tujuan: prev.tujuan.filter(
          (item) =>
            !(item.tujuan_id === tujuan_id && item.tujuan_type === tujuan_type),
        ),
      };
    });
  }
      ...
      >
        <form onSubmit={handleSubmit}>
          <DialogHeader className="gap-0">
            ...
            <DialogDescription>
              Pilih tujuan pengiriman untuk hari ini. Klik simpan saat selesai.
            </DialogDescription>
          </DialogHeader>
          <Button className="my-4" type="button" onClick={handleToggleAll}>
              ...
              {leftColumn.map((el) => (
                <div key={`${el.tujuan_type}-${el.id}`} className="flex gap-2">
                  <Checkbox
                    ...
                    checked={form.tujuan.some(
                      (item) =>
                        item.tujuan_id === el.id &&
                        item.tujuan_type === el.tujuan_type,
                    )}
                  ...
                  />
                  <Label htmlFor={`${el.tujuan_type}-${el.id}`}>
                    {el.nama}
                  </Label>
                </div>
              ))}
