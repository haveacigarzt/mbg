...
interface DialogTambahPedagangProps {
  onPedagangCreated: () => void;
  children: React.ReactNode;
  sppg_id: number;
}

const DialogTambahPedagang = ({ onPedagangCreated, children, sppg_id }: DialogTambahPedagangProps) => {
  const [open, setOpen] = useState(false);
  const initialForm = { nama: '', alamat: '', no_hp: '', jenis_produk: '', longitude: '', latitude: '' };
      ...
      >
        <form onSubmit={handleSubmit}>
          <DialogHeader className="gap-0">
            <DialogTitle className="text-lg font-semibold">Tambah data Pedagang Lokal</DialogTitle>
            <DialogDescription>Lengkapi data Pedagang Lokal baru. Klik tambah saat selesai.</DialogDescription>
          </DialogHeader>
...
export default DialogTambahPedagang;
